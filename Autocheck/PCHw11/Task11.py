...
class Iterable:
    ...
    def __next__(self):
        while self.current_index < self.max_vectors:
            self.current_index += 1
            s1 = randrange(1, self.max_points+1)
            s2 = randrange(1, self.max_points+1)
            res = Vector(Point(s1, s2))
...
class RandomVectors:
    ...
    def __iter__(self):
        return Iterable(max_vectors=self.max_vectors, max_points=self.max_points)

for i in RandomVectors(5,10):
    print(i)
